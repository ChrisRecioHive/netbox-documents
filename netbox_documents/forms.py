...
from dcim.models import Cable, Site, Location, Device, DeviceType
...
from .models import (
    ...
    DeviceTypeDocTypeChoices,
)


#### Cable Document Form & Filter Form
...
class SiteDocumentFilterForm(NetBoxModelFilterSetForm):
    ...
    site = forms.ModelMultipleChoiceField(queryset=Site.objects.all(), required=False)
...
class LocationDocumentForm(NetBoxModelForm):
    ...
    location = DynamicModelChoiceField(
        queryset=Location.objects.all(), query_params={"site_id": "$site"}
    )
...
class LocationDocumentFilterForm(NetBoxModelFilterSetForm):
    ...
    document_type = forms.MultipleChoiceField(
        choices=LocationDocTypeChoices, required=False
    )
...
class DeviceDocumentFilterForm(NetBoxModelFilterSetForm):
    ...
    document_type = forms.MultipleChoiceField(
        choices=DeviceDocTypeChoices, required=False
    )
...
class DeviceTypeDocumentFilterForm(NetBoxModelFilterSetForm):
    ...
    document_type = forms.MultipleChoiceField(
        choices=DeviceTypeDocTypeChoices, required=False
    )
...
class CircuitDocumentFilterForm(NetBoxModelFilterSetForm):
    ...
    circuit = forms.ModelMultipleChoiceField(
        queryset=Circuit.objects.all(), required=False
    )

    document_type = forms.MultipleChoiceField(
        choices=CircuitDocTypeChoices, required=False
    )
